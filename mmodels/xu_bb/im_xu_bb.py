...
class im_xu_bb(object):
    ...
    #def im_bmodel_odes(self,t,y,kla,pabs,yO,Sf:float,mu_set:float,V_fixed:bool=False,X_fixed:bool=False,D_set:float=0,pulse_cycle_time:float=None,pulse_on_ratio:float=None,anae_ratio_comp:float=None,returns:str='dydt'):
    def im_bmodel_odes(self,t,y,kla,pabs,yO,method,pulse_cycle_time:float,pulse_on_ratio:float,anae_ratio_comp:float,Sf:float,mu_set:float,D_set:float,V_fixed:bool=False,X_fixed:bool=False,returns:str='dydt'):
        ...
        A =  y[3]
        #print(kla*yO)
        #print(anae_ratio_comp*pulse_cycle_time*pulse_on_ratio)
        print(method)
        if method==0:
            kla=kla
        elif method==1:
            kla=kla
            if np.all((t>=(1-anae_ratio_comp)*pulse_on_ratio*pulse_cycle_time/3600) & (t<=pulse_on_ratio*pulse_cycle_time/3600)):
            #if np.all((t>=157.68/3600) & (t<=228/3600)):
            #0.3*600/3600<t<0.4*600/3600:
                kla=0
                print('hello')        
        # rates equations
        ...
        if returns == 'dydt':
            ...
            #if (pulse_on_ratio-(anae_ratio_comp*pulse_on_ratio))*pulse_cycle_time/3600<t<pulse_on_ratio*pulse_cycle_time/3600:
