...
class im_anane_bb(object):
    ...
    def __init__(self,strain_id:str=None,strain_description:str=None):
        ...
        self.rates['description'] = [
            ...
            'specific acetate excretion rate',
    ...
    def define_strain_params(self,qS_max,qm_max,qA_max,qO_max,pA_max,Ysx_ox,Ysx_of,Ysa,Yax,Ki_s,Ki_a,Ks,Ksa,Kap,Ko,Ki_o):
        '''Description goes here.'''
        ...
        self.Ca = 30    # [gA/CmolA]
        self.Yso = 33.3 # [mmolO/gS] 33.3
        self.Yao = 33.3 # [mmolO/gS] 33.3
    ...
    def im_bmodel_odes(self,t,y,kla,pabs,yO,method,pulse_cycle_time:float,pulse_on_ratio:float,anae_ratio_comp:float,Sf:float,mu_set:float,D_set:float,V_fixed:bool=False,X_fixed:bool=False,returns:str='dydt'):
        '''Description goes here.'''
        ...
        qS = self.qS_max/(1+A/self.Ki_a)*(S)/(S+self.Ks)

        #qm = self.qm_max
        ### qS_of
        qS_of=self.pA_max*qS/(qS+self.Kap)
        ### qS_ox
        qS_ox=(qS-qS_of)*O/(O+self.Ko)
        ### actual maintenance coefficient
        #qm = min(self.qm_max,qS)
        qm = np.minimum(self.qm_max,qS_ox)
        ###qS_A
        qS_A=self.qA_max/(1+qS/self.Ki_s)*A/(A+self.Ksa)
        ###pA
        pA=qS_of*self.Ysa
        ###qA
        qA=pA-qS_A
        ###qO
        qO=(qS_ox-qm)*self.Yso+qS_A*self.Yao
        #qO=(qS_ox-self.qm_max)*self.Yso+qS_A*self.Yao
        ###mu
        #mu=(qS_ox-qm)*self.Ysx_ox+qS_of*self.Ysx_of+qS_A*self.Yax
        mu=(qS_ox-self.qm_max)*self.Ysx_ox+qS_of*self.Ysx_of+qS_A*self.Yax
        #mu=(qS_ox-qm)*self.Ysx_ox+qS_of*self.Ysx_of+qS_A*self.Yax
        ...
        ### specific oxygen uptake rate for substrate oxidation in the oxidative energy metabolism
        #qO_s = min(qS_ox_en_th*self.Yso,OUC)
        ### specific substrate uptake rate of the energy oxidative metabolism
        ...
        ### specific acetate consumption rate for the energy metabolism
        qA_c_en = min(qA_c_th - qA_c_an_th,(OUC - qO_s)/self.Yao)
        ### specific acetate consumption rate for the anabolism
